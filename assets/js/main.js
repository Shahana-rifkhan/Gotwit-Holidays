/* ============================================================
   ...
   /* ============================================================
      GODWIT HOLIDAYS — Header live deal search
      Shows matching holiday deals while a destination is typed in the
      header search box. The deal data is loaded automatically on pages
      that do not already include deals-data.js.
      ============================================================ */
   (function(){
     const wraps=$$('.nav-search');
     if(!wraps.length) return;
   
     const currentScript=document.currentScript;
     const scriptUrl=currentScript?.src || '';
     const dealsDataUrl=scriptUrl ? new URL('deals-data.js',scriptUrl).href : 'assets/js/deals-data.js';
     const siteRoot=scriptUrl ? new URL('../../',scriptUrl).href : './';
     const dealHref=id=>new URL(`holidays/deal-${String(id).padStart(2,'0')}.html`,siteRoot).href;
     const allDealsHref=q=>new URL(`holidays.html?q=${encodeURIComponent(q)}`,siteRoot).href;
   
     function ensureDeals(){
       if(Array.isArray(window.GODWIT_DEALS)) return Promise.resolve(window.GODWIT_DEALS);
       if(window.__godwitDealsPromise) return window.__godwitDealsPromise;
       window.__godwitDealsPromise=new Promise((resolve,reject)=>{
         const tag=document.createElement('script');
         tag.src=dealsDataUrl;
         tag.async=true;
         tag.onload=()=>resolve(window.GODWIT_DEALS||[]);
         tag.onerror=()=>reject(new Error('Unable to load holiday deals data.'));
         document.head.appendChild(tag);
       });
       return window.__godwitDealsPromise;
     }
   
     const style=document.createElement('style');
     style.id='gwHeaderSearchStyles';
     style.textContent=`
   .nav-search{z-index:80}
   .gw-search-dropdown{position:absolute;right:52px;top:calc(50% + 31px);width:min(420px,calc(100vw - 32px));background:#fff;border:1px solid rgba(11,61,120,.13);border-radius:16px;box-shadow:0 18px 46px rgba(11,61,120,.2);padding:8px;opacity:0;visibility:hidden;transform:translateY(-5px);transition:opacity .18s ease,transform .18s ease,visibility .18s;max-height:min(470px,65vh);overflow:auto;z-index:90}
   .nav-search.open .gw-search-dropdown.gw-show{opacity:1;visibility:visible;transform:translateY(0)}
   .gw-search-result{display:grid;grid-template-columns:66px minmax(0,1fr);gap:11px;align-items:center;padding:9px;border-radius:12px;text-decoration:none;color:inherit;transition:background .16s ease,transform .16s ease}
   .gw-search-result:hover,.gw-search-result.gw-active{background:#f3f8fd;transform:translateX(2px)}
   .gw-search-thumb{width:66px;height:53px;border-radius:9px;overflow:hidden;background:#edf3f9}
   .gw-search-thumb img{width:100%;height:100%;object-fit:cover;display:block}
   .gw-search-copy{min-width:0}
   .gw-search-title{display:block;color:var(--blue-deep,#0b3d78);font-size:.88rem;font-weight:750;line-height:1.3;margin-bottom:4px}
   .gw-search-meta{display:flex;gap:5px;flex-wrap:wrap;color:#69809d;font-size:.73rem;line-height:1.25}
   .gw-search-price{color:var(--pink,#d41474);font-weight:750}
   .gw-search-empty{padding:18px 15px;text-align:center;color:#6d7f95;font-size:.84rem;line-height:1.45}
   .gw-search-footer{display:flex;align-items:center;justify-content:space-between;gap:10px;margin:5px 4px 1px;padding:10px 9px 5px;border-top:1px solid #edf1f6;color:#667c96;font-size:.76rem}
   .gw-search-footer a{color:var(--pink,#d41474);font-weight:750;text-decoration:none;white-space:nowrap}
   .gw-search-footer a:hover{text-decoration:underline}
   @media(max-width:820px){.gw-search-dropdown{right:0;top:calc(100% + 62px);width:min(360px,calc(100vw - 32px));max-height:58vh}}
   `;
     if(!document.getElementById(style.id)) document.head.appendChild(style);
   
     const esc=v=>String(v??'').replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]));
     const norm=v=>String(v??'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();
     const resultHtml=d=>`<a class="gw-search-result" href="${dealHref(d.id)}" role="option" aria-label="${esc(d.title)}"><span class="gw-search-thumb"><img src="${esc(d.image)}" alt="" loading="lazy"></span><span class="gw-search-copy"><span class="gw-search-title">${esc(d.title)}</span><span class="gw-search-meta"><span>${esc(d.nights)} nights</span><span>•</span><span class="gw-search-price">${esc(d.price)}</span><span>•</span><span>${esc(d.category)}</span></span></span></a>`;
   
     wraps.forEach((wrap,idx)=>{
       const form=$('.nav-search-form',wrap), input=$('input[name="q"]',form);
       if(!form||!input) return;
   
       const dropdown=document.createElement('div');
       dropdown.className='gw-search-dropdown';
       dropdown.id=`gwSearchDropdown${idx}`;
       ...
       function closeResults(){
         dropdown.classList.remove('gw-show');
         active=-1;
         input.removeAttribute('aria-activedescendant');
       }
   
       function setActive(next){
         const items=$$('.gw-search-result',dropdown);
         if(!items.length){active=-1;return;}
         active=(next+items.length)%items.length;
         items.forEach((el,i)=>el.classList.toggle('gw-active',i===active));
       ...
       async function updateResults(){
         ...
         const q=norm(raw);
         if(q.length<2){closeResults();dropdown.innerHTML='';return;}
         try{
           const deals=await ensureDeals();
           matches=deals.map(d=>{
             const title=norm(d.title), desc=norm(d.description);
             let score=99;
             if(title.startsWith(q)) score=0;
             else if(title.split(/[^a-z0-9]+/).includes(q)) score=1;
             else if(title.includes(q)) score=2;
             else if(desc.includes(q)) score=3;
             return {d,score};
           }).filter(x=>x.score<99).sort((a,b)=>a.score-b.score||a.d.id-b.d.id).map(x=>x.d);
   
           active=-1;
           if(!matches.length){
             dropdown.innerHTML=`<div class="gw-search-empty">No holiday deals currently match <strong>${esc(raw)}</strong>.<br>Try another destination.</div>`;
           }else{
             const visible=matches.slice(0,6);
             dropdown.innerHTML=visible.map(resultHtml).join('')+`<div class="gw-search-footer"><span>${matches.length} matching deal${matches.length===1?'':'s'}</span><a href="${allDealsHref(raw)}">View all results →</a></div>`;
           }
           dropdown.classList.add('gw-show');
         }catch(err){
           dropdown.innerHTML='<div class="gw-search-empty">Search suggestions are temporarily unavailable. Press Enter to search all holidays.</div>';
           dropdown.classList.add('gw-show');
         }
       }
   
       let timer;
       input.addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(updateResults,70)});
       input.addEventListener('focus',()=>{if(input.value.trim().length>=2) updateResults();else ensureDeals().catch(()=>{})});
       input.addEventListener('keydown',e=>{
         if(!dropdown.classList.contains('gw-show')) return;
         if(e.key==='ArrowDown'){e.preventDefault();setActive(active+1)}
         else if(e.key==='ArrowUp'){e.preventDefault();setActive(active-1)}
         else if(e.key==='Enter'&&active>=0){e.preventDefault();$$('.gw-search-result',dropdown)[active]?.click()}
     ...
     // Preload the small search index so suggestions feel instant on first use.
     ensureDeals().catch(()=>{});
   })();
   ...
   `;
     ...
     // ===============================
   // Dynamic Favicon - Godwit Holidays
   // ===============================
   (function () {
     const faviconPath = "assets/img/icon.png"; // change path if your icon location is different
